...
concurrent loading capabilities, make the platform well equipped to
...
up to about 5 million triples on standard (notebook) hardware and deal
...
channels \const{new_literal} and \const{old_literal} to maintain an
...
database, nor added to the persistent store. Different merging options
may be supported through the \arg{Options} argument later.  Currently
defined options are:

    \begin{description}
	\termitem{concurrency}{+PosInt}
Number of threads used to reload database and journals from the
...
If \const{true}, suppress loading messages from rdf_attach_db/2.
...
Change persistency of named database (4th argument of rdf/4). By default
all databases are persistent. Using \const{false}, the journal and
...
True if \arg{Resource} is an individual of \arg{Class}.  This implies
...
infrastructure. Nevertheless, version~3 is almost perfectly upward
...
changes with more complicated control structures that combine
...
committing a transaction steps the generation only by one.
